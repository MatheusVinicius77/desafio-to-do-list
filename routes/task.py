...
async def get_task(task_id: int, session: Session = Depends(get_db)) -> TaskSchema:
...
async def get_all_tasks(session : Session = Depends(get_db)) -> List[TaskSchema]:
...
async def create_task(new_task:TaskCreateSchema , session : Session = Depends(get_db)) -> TaskSchema:
    return task.create_task(session, new_task)


# UPDATE TASK
@router.put("/update/{task_id}", response_model=TaskSchema)
async def update_task(task_id : int , change_task:TaskUpdateSchema , session : Session = Depends(get_db)) -> TaskSchema:
    return task.update_task(session, task_id, change_task)


# DELETING A TASK BY ID
@router.delete("/delete/{task_id}", response_model=Dict[str, str])
async def delete_task_by_id(task_id: int, session: Session = Depends(get_db)) -> Dict[str,str]:
